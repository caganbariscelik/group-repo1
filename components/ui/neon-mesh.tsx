...
export interface NeonMeshProps {
    ...
    className?: string;
    /**
     * "hero" renders the full-bleed title/subtitle overlay (home page takeover).
     * "backdrop" renders just the animated grid, fixed behind page content —
     * used site-wide so every route sits on the same living mesh.
     */
...
export function NeonMesh({
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            const lineAlphaScale = variant === "backdrop" ? 0.6 : 1;
            ...
            for (let i = 0; i < points.length; i++) {
                ...
                const ambientZ =
                    Math.sin(p.baseX * 0.015 + p.baseY * 0.015 + time) * 18;
            ...
            for (let i = 0; i < constraints.length; i++) {
                ...
                ctx.strokeStyle = isHot
                    ...
                    : `rgba(${baseMeshColor}, ${Math.min(
                        1,
                        Math.max(0.1, 0.25 * avgScale * lineAlphaScale)
                    )})`;
    ...
    return (
        <div
            ...
            className={
                isBackdrop
                    ? `fixed inset-0 -z-10 h-screen w-screen overflow-hidden select-none bg-black ${className}`
                    : `relative w-full h-screen overflow-hidden select-none bg-black ${className}`
            }
        >
            <canvas
                ref={canvasRef}
                className={`absolute inset-0 block ${isBackdrop ? "" : "cursor-crosshair"}`}
            />
